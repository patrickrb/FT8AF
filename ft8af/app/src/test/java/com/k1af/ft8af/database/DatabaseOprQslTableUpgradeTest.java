...
/**
 * The QSLTable upgrade path — that an <em>existing</em> database carries every column
 * {@link DatabaseOpr#doInsertQSLData} writes after the helper opens (and upgrades) it.
 *
 * <p>This exists because of a shipped crash: a column was once added to the
 * {@code CREATE TABLE} and to the {@code alterTable} block, but the schema version was left
 * unchanged — and {@code onUpgrade} (the only thing that runs those ALTERs on an existing
 * database) fires solely when that number increases. The result was invisible in
 * development and fatal in the field: a fresh install got the column from CREATE TABLE and
 * behaved perfectly, while every upgraded install kept the old table and died with
 * "table QSLTable has no column named &lt;x&gt;" on the first logged QSO.
 *
 * <p>So the meaningful test is not "does a new database have the columns" — that passed
 * throughout the bug. It is "does a database created by the <em>previous</em> version have
 * them after the helper opens it", which is what
 * {@link #upgradedTableCarriesEveryColumnTheInsertNames} checks by building a legacy table
 * by hand and then letting DatabaseOpr upgrade it.
 */
...
public class DatabaseOprQslTableUpgradeTest {
    ...
    public void schemaVersionIsAheadOfTheReleaseThatShippedWithoutTheColumns() {
        // 19 is the version that shipped a new column in CREATE TABLE but never ran the
        // ALTER. Anything at or below it leaves upgraded installs crashing on every QSO.
        assertThat(DatabaseOpr.SCHEMA_VERSION).isGreaterThan(19);
    }

    @Test
    public void upgradedTableCarriesEveryColumnTheInsertNames() {
        // Guards the next occurrence: doInsertQSLData writes these column names, so an
        // upgraded database missing any of them crashes the app on the first logged QSO.
        // Adding a column to that INSERT without a migration should fail here, not in a car.
