...
/**
 * The CQ modifier a running trip transmits: `CQ RTOA <call> <grid>`.
 *
 * **Why not "RTOTA".** An FT8 standard message packs the CQ into the 28-bit
 * `c28` field, whose reserved range encodes `CQ` plus *one to four letters* (or
 * three digits) — that is the whole vocabulary the format has. `POTA` fits at
 * exactly four; `RTOTA` is five and simply has no encoding. The app enforces
 * this at [com.k1af.ft8af.Ft8Message] (`[0-9]{3}|[A-Z]{1,4}`), where an
 * over-long modifier is dropped and the CQ goes out bare — audible, decodable,
 * and silently missing the thing that made it a road-trip CQ.
 *
 * The alternative, free text, is capped at 13 characters and carries no grid at
 * all: `CQ RTOTA KS3CKC` is 15 and does not fit, and even where it does fit no
 * receiver's CQ filter treats free text as a CQ. So the on-air token is `RTOA`
 * and the message stays a real standard CQ, grid included.
 */
const val RTOTA_CQ_MODIFIER = "RTOA"
 ...
 * killed) must not record `RTOA` as the thing to put back — that would make the
 * road-trip token outlive the road trip, and every later CQ would keep claiming
 * a trip that ended.
 */
fun modifierToRemember(current: String?): String = if (current == RTOTA_CQ_MODIFIER) "" else current.orEmpty()
 ...
 * saved `RTOA` and set `POTA`; if ending the trip blindly wrote the pre-trip
 * value back, the operator would be mid-park-activation transmitting a plain CQ,
 * and POTA's own end would then restore `RTOA` on top — a token for a trip that
...
fun modifierAfterTripEnd(
    current: String?,
    remembered: String,
): String? = if (current == RTOTA_CQ_MODIFIER) remembered else null
 ...
 * later.
 */
object RtotaCqSession {
    private const val TAG = "RtotaCqSession"
    ...
    val isApplied: Boolean get() = GeneralVariables.toModifier == RTOTA_CQ_MODIFIER

    /**
     * Impose `RTOA` for a running trip. Idempotent: calling it again while it is
    ...
    fun apply() {
        ...
        if (current == RTOTA_CQ_MODIFIER) {
            log("apply — already set")
            return
        }
        remembered = modifierToRemember(current)
        GeneralVariables.toModifier = RTOTA_CQ_MODIFIER
        log("apply modifier='$RTOTA_CQ_MODIFIER' remembered='$remembered'")
    }
    ...
    private fun log(msg: String) {
        ...
        try {
            ...
            FileWriter(File(dir, "debug.log"), true).use { it.append("$ts RtotaCq: $msg\n") }
        } catch (_: Exception) {
        }
    }
}
