 ...
 * spans exactly one slot: {@code max = slotMillis}, {@code progress = utcMs mod slotMillis}
 * (in milliseconds). For FT8 that is {@code max = 15000}, byte-for-byte the same sweep the
 * old {@code (utcMs/1000) % 15} bar drew (both reach full at the slot boundary), just at
 * millisecond rather than whole-second resolution — and the timer that drives it only ticks
 * about once per second, so the visible cadence is unchanged.
 */
public final class SlotProgressBar {
    ...
    /**
     * The {@code ProgressBar} max for a slot of the given length. Falls back to the FT8
     * 15&nbsp;s cycle for a non-positive length so {@link #progressFor(long, long)} can never
     * divide by zero.
     */
    public static int maxFor(long slotMillis) {
        return (int) (slotMillis > 0 ? slotMillis : FT8_SLOT_MILLIS);
    }
